// Sample generator for the quadrature-free even-N closed-form LCD path
// (gm_to_dirac_even_closed_form). Mirrors generate_samples.cpp, but:
//
//   - N must be EVEN (the closed form has no odd-N case);
//   - the target is a STANDARD NORMAL or an ISOTROPIC sigma^2 * I, so a single
//     sigma is taken rather than a per-axis diagonal;
//   - bMax is a double, because the isotropic reduction solves the
//     standard-normal problem at bMax / sigma, which is rarely an integer.
//
